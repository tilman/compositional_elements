...
def get_cone_combination_intersections(pose_directions: Sequence[PoseDirection]) -> Sequence[Tuple[int, Combination, Polygon]]:
    ...
    for combination_length in range(1,len(pose_directions)+1):
        cone_combinations: Sequence[Combination] = list(itertools.combinations(range(0,len(pose_directions)), combination_length))
        ...
        for combination in cone_combinations:
            ...
            for i in combination[1:]:
                ...
                combi_intersection_result = combi_intersection_result.intersection(pose_directions[i].cone)
            if not combi_intersection_result.is_empty:
                ...
                combination_intersections.append((len(combination), combination, combi_intersection_result))
    return combination_intersections

def get_filtered_cone_intersections(poses) -> Sequence[Polygon]:
    ...
    filtered_combi_length = combination_intersections[-1][0]
    filtered_cone_intersections = [intersect[2] for intersect in combination_intersections if intersect[0] == filtered_combi_length]
