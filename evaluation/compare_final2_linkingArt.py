...
from compoelem.detect.openpose.lib.utils.common import Human, CocoPart
import pickle
import os
...
def calc_ransac_inlier_threshold(r,s):
    ...
    return 0.01 # TODO implement function
...
def robust_verify(poses_i1, poses_i2, neck_norm=False):
    poses_i1 = np.array([openpose_to_nparray(human) for human in poses_i1]) # output shape of each item is (18, 2) since we are using the 18 openpose keypoint model
    poses_i2 = np.array([openpose_to_nparray(human) for human in poses_i2])
...
def compare(data, compare_method, with_robust_verify):
    res_metrics = {}
    precision_curves = {}
    all_retrieval_res = []
    for query_data in tqdm(data, total=len(data)):
        ...
        if with_robust_verify == True:
            ...
            for target in sorted_compare_results[0:l]:
                ...
                if query_data["className"] == target_data["className"] and query_data["imgName"] == target_data["imgName"]:
                    continue
                match_count = robust_verify(query_data["compoelem"]["humans"], target_data["compoelem"]["humans"], neck_norm=False)
...
def eval_single_combination(arg_obj):
    ...
    with_robust_verify = arg_obj["with_robust_verify"]    
    
    if compare_method_name == "compare_dist_bipart":
        compare_method = compare_dist_bipart
    elif compare_method_name == "compare_dist_min":
        compare_method = compare_dist_min
    else:
        raise NotImplementedError("compare not implemented")


    start_time = datetime.datetime.now()
    eval_dataframe, precision_curves, all_retrieval_res = compare(list(datastore.values()), compare_method, with_robust_verify)
    filename = "final2_time_{}_latp_{}_withVerify{}.pkl".format(
    ...
    res_summary = {
        ...
        "with_robust_verify": with_robust_verify,
    }
